...
class Details {
  ...
  static getNotesByTicketId(ticketId) {
    return db.any('select * from notes where all_tickets_id = $1', 
    [parseInt(ticketId)]).then(arrayOfNotes => {
        console.log(arrayOfNotes);
        return arrayOfNotes;
      });
  }
  ...
  static newDetailSubmitted(notecontent, users_id, all_tickets_id) {
    ...
    const entireDate = `${realMonth}/${date}/${year}
                        ${hour}:${minute}:${second}`
    return db.none('insert into notes (note_content, users_id, all_tickets_id, time_posted) values ($1, $2, $3, $4)',
    [notecontent, users_id, all_tickets_id, entireDate]); 
  }
}
